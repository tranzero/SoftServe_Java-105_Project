...
public class TicketsManagerImpl implements TicketsManager {
	...
	/**
	 * Finds Ticket by Name.
	 */
	@Transactional(readOnly = true)
	@Override
	public Tickets findByTicketName(String ticketName) {

		try {
			return ticketsDao.findByName(ticketName);
		} catch (RuntimeException e) {
			RuntimeException ex = new StationManagerException(
					"Could not find Tickets by name", e);
			LOGGER.error(e);
			LOGGER.error(ex);
			throw ex;
		}
	...
	public void createTicket(String ticketName, Integer orderId,
		...
		Tickets ticket = new Tickets(ticketName, ordersDao.findById(orderId),
				tripsDao.findById(tripId), customerFirstName, customerLastName,
				seatType);
	...
	public Tickets getTicket(String ticketName, Trips trip,
		...
		return new Tickets(ticketName, trip, customerFirstName,
				customerLastName, seatType);
