...
@DependsOn({ "sourceDao", "pathUtil", "loggerProgrammaticalConfigurator" })
public class PreferenceUtil {
	private static final Logger LOGGER = Logger.getLogger(PreferenceUtil.class);

	private static final String PREFERENCE_NODE = "LV102JAVA";
	...
	public void createSourcesPaths() {
		// Create paths for existing sources
		for (Source source : sourceDao.getAllEntities()) {
			String dirName = source.getDirname();
			if (dirName == null || dirName.equalsIgnoreCase("null")) {
				continue;
			}
			String sourcePath = this
					.getAbsolutePath(pref
							.get(PropertyKeys.SOURCES_PATH_PROPERTY_NAME,
									standartValues
											.get(PropertyKeys.SOURCES_PATH_PROPERTY_NAME)))
					+ dirName;

			this.createPath(sourcePath);
		}
	}
