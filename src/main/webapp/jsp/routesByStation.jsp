...
<section id="content">
	...
	<form:form action="routesTrips" method="get">
		...
		<spring:message code="label.navigation.arrival" /> <input name="findBy" type="radio" value="findByArr" checked></input>
        <spring:message code="label.navigation.departure" /> <input name="findBy" type="radio" value="findByDep"></input>
        <p/>
	...
	<c:if test="${not empty param.nameStation  && not empty param.timeMin && not empty param.timeMax}">
		<script>
			function showRoutesTripPage(stationName_, timeMin_, timeMax_, findBy_, pageNumber_,
					resultsPerPage_) {
			...
			$(window).load(
					function() {
						showRoutesTripPage("${param.nameStation}", "${param.timeMin}", "${param.timeMax}", "${param.findBy}", 
